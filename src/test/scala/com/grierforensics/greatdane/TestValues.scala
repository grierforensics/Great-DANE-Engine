...
object TestValues {
  ...
  // Sample DANE Entry for testing (email, various certificate formats, etc.)
  object Entry {
  ...
  val TestDaneEntryCount = 3
  val TestDaneEntries = Map(Entry.domain -> Seq.fill(TestDaneEntryCount)(Entry.dane))
  ...
  class TestDaneEntryFetcherFactory extends DANEEntryFetcherFactory {
    override def build(domainName: String): DANEEntryFetcher = new DANEEntryFetcher {
      override def getEntries: java.util.List[_] = {
